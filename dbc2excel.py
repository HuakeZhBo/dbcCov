...
msg_page_name = "Matrix"
node_page_name = "Nodes"
title_row = 0
signal_name_col = 6

msg_excel_title = ('Msg Name\n报文名称', 'Msg Type\n报文类型', 'Msg ID\n报文标识符', 'Msg Send Type\n报文发送类型',
                ...
                "Msg Cycle Time Fast(ms)\n报文快速发送周期(ms)", "Msg Nr. Of Reption\n报文快速发送次数", "Msg Delay Time(ms)\n报文延时时间(ms)",
                )
...
class DbcLoad(object):
    def __init__(self, dbc_name_in):
        ...
        if self.dbc_fd.readable():
            ...
            self.dbc_list = []
            self.node = {}
            self.dbc_name = dbc_name_in.split("\\")[-1]
            self.dbc_cycle_time = {}
            self.tran_recv = set()
            self.tran_recv_list = []
    ...
    def parse_dbc(self, if_show, val_description_max_number = 70):
        ...
        if(True):
            ...
            while i < length:
                if len(line_list[i].split()) > 2:
                    if line_list[i].split()[0] == 'BU_:':
                        node_index = 1
                        while node_index < len(line_list[i].split()):
                            self.node[line_list[i].split()[node_index]] = ''
                            node_index += 1
                    elif line_list[i].split()[0] == 'CM_' and line_list[i].split()[1] == 'BU_':
                        self.node[line_list[i].split()[2]] = line_list[i].split('"')[1]
                i += 1
    ...
    def dbc_excel_gen(self):
        ...
        msg_sheet = book.add_sheet(msg_page_name, cell_overwrite_ok = True)
        row_counter = 0

        #write title
        title_len = len(msg_excel_title)
        i = 0
        while i < title_len:
            msg_sheet.write(title_row, i, msg_excel_title[i], set_style(0x28, True, True))
            i = i+1
        tran_recv_len = len(self.tran_recv_list)
        tmp = i + tran_recv_len
        j = 0
        while i < tmp:
            msg_sheet.write(title_row, i, self.tran_recv_list[j], set_style(0x28, True, True))
            i += 1
            j += 1

        #调整行距离
        col0 = msg_sheet.col(0)
        col0.width = 300*20
        col0 = msg_sheet.col(1)
        col0.width = 120*20
        col0 = msg_sheet.col(2)
        col0.width = 200*20
        col0 = msg_sheet.col(5)
        col0.width = 120*20
        col0 = msg_sheet.col(6)
        col0.width = 400*20
        col0 = msg_sheet.col(7)
        col0.width = 400*20
        col0 = msg_sheet.col(9)
        col0.width = 120*20
        col0 = msg_sheet.col(12)
        col0.width = 120*20
        col0 = msg_sheet.col(13)
        col0.width = 120*20
        col0 = msg_sheet.col(16)
        col0.width = 150*20
        col0 = msg_sheet.col(17)
        col0.width = 150*20
        col0 = msg_sheet.col(18)
        col0.width = 150*20
        col0 = msg_sheet.col(19)
        col0.width = 150*20
        col0 = msg_sheet.col(22)
        col0.width = 120*20
        col0 = msg_sheet.col(24)
        col0.width = 700*20
        #write BO units
        dbc_length = len(self.dbc_list)
            ...
            for bo_unit in self.dbc_list[i]:#for bo_unit in self.dbc_list[i]:
                if 'message_id' in bo_unit:
                    ...
                    if 'cycle_time' in bo_unit:
                        ...
                        msg_sheet.write(title_row + row_counter, 4, 'NULL', set_style(0x0D, True, True))
                    msg_sheet.write(title_row + row_counter, 5, bo_unit['message_size'], set_style(0x28, True, True))
                    #发送者和接收者
                    k = 0
                    tran_recv_len = len(self.tran_recv_list)
                    while k < tran_recv_len:
                        if 'transmitter' in bo_unit:
                            if bo_unit['transmitter'].find(self.tran_recv_list[k]) != -1:
                                msg_sheet.write(title_row + row_counter, signal_name_col + 22 + k, 'S', style_index)
                            else:
                                msg_sheet.write(title_row + row_counter, signal_name_col + 22 + k, '', style_index)
                        k += 1
                if 'signal_name' in bo_unit:
                    row_counter = row_counter + 1
                    #信号名称
                    msg_sheet.write(title_row + row_counter, signal_name_col, bo_unit['signal_name'],style)
                    #信号描述
                    if 'comment' in bo_unit:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 1, bo_unit['comment'],style_index)
                    else:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 1, '', style_index)
                    #字节序
                    if bo_unit['byte_order'] == 0:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 2, "Motorola LSB",style_index)
                    else:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 2, "Intel",style_index)
                    #起始字节
                    msg_sheet.write(title_row + row_counter, signal_name_col + 3, str(int(bo_unit['true_start_bit'] / 8)),style_index)
                    #起始位
                    msg_sheet.write(title_row + row_counter, signal_name_col + 4, str(bo_unit['true_start_bit']),style_index)
                    #循环类型
                    msg_sheet.write(title_row + row_counter, signal_name_col + 5, 'cycle',style_index)
                    #信号类型
                    msg_sheet.write(title_row + row_counter, signal_name_col + 6, str(bo_unit['signal_size']),style_index)
                    #数据类型
                    if bo_unit['value_type'] == 0:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 7, 'unsigned',style_index)
                    else:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 7, 'signed',style_index)
                    #信号精度
                    msg_sheet.write(title_row + row_counter, signal_name_col + 8, str(bo_unit['factor']),style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 9, str(bo_unit['offset']),style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 10, str(bo_unit['minimum']),style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 11, str(bo_unit['maximum']),style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 12, '0x00',style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 13, str(hex(2**bo_unit['signal_size'] - 1)),style_index)
                    #初始值
                    if 'inital_value' in bo_unit:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 14, bo_unit['inital_value'],style_index)
                    else:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 14, '',style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 15, '',style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 16, '',style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 17, bo_unit['unit'],style_index)
                    #信号描述
                    if 'val_description' in bo_unit:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 18, bo_unit['val_description'],style_index)
                    else:
                        msg_sheet.write(title_row + row_counter, signal_name_col + 18, '',
                                    style_index)
                    #周期次数延时
                    msg_sheet.write(title_row + row_counter, signal_name_col + 19, '',style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 20, '',style_index)
                    msg_sheet.write(title_row + row_counter, signal_name_col + 21, '',style_index)
                    #发送者和接收者
                    k = 0
                    tran_recv_len = len(self.tran_recv_list)
                    while k < tran_recv_len:
                        if 'receiver' in bo_unit:
                            if bo_unit['receiver'].find(self.tran_recv_list[k]) != -1:
                                msg_sheet.write(title_row + row_counter, signal_name_col + 22 + k, 'R', style_index)
                            else:
                                msg_sheet.write(title_row + row_counter, signal_name_col + 22 + k, '', style_index)
                        k += 1
            i = i+1

        nodes_sheet =  book.add_sheet(node_page_name, cell_overwrite_ok = True)
        nodes_sheet.write(title_row, 0, 'Node Name\n结点名称' , set_style(0x28, True, True))
        nodes_sheet.write(title_row, 1, 'Node Comment\n节点描述' , set_style(0x28, True, True))
        nodes_sheet.col(0).width = 300*20
        nodes_sheet.col(1).width = 400*20
        node_index = 1
        for node,node_comment in self.node.items():
            nodes_sheet.write(node_index, 0, node, set_style(255, True, True))
            nodes_sheet.write(node_index, 1, node_comment, set_style(255, False, False))
            node_index += 1
        book.save(self.dbc_name.split('.')[0] + '.xls')
